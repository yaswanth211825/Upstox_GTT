...
class AISignalParser:
    ...
    def __init__(self, api_key: str, provider: str = "GEMINI", model: Optional[str] = None):
        ...
        if self.provider == "GEMINI":
            ...
            self.model = genai.GenerativeModel(self.model_name or 'gemini-2.0-flash-lite')
    ...
    def _calculate_expiry(self, instrument: str) -> str:
        ...
        if instrument.upper() in ("NIFTY", "BANKNIFTY"):
            # Tuesday expiry (weekday 1)
            target_weekday = 1
        else:  # SENSEX and others
...
5. Entry Price: Can be a range (e.g., 350-360) or single value (e.g., 350)
...
8. Expiry: Date in format "7th NOVEMBER" (if not present, leave as null)
...
If ANY required field is missing or unclear, return: {{"error": "missing_field", "missing": "field_name"}}
